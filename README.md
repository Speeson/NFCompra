...
- La PWA muestra una landing pública en `/` para visitantes anónimos, con acceso a las rutas directas de registro, inicio de sesión, verificación, reenvío, recuperación y restablecimiento. La landing presenta NFC como una capacidad próxima; los flujos de autenticación directos se mantienen sin cambios. Conserva el token de acceso en memoria y la API gestiona la cookie de renovación. Permite seleccionar hogares y listas, crear hogares/listas y gestionar productos; consulta la lista visible cada 15 segundos y aplica mutaciones optimistas con reintento ante conflictos.
