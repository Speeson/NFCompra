...
- La PWA muestra una landing pública en `/` para visitantes anónimos, con modales accesibles para iniciar sesión y crear cuenta. El foco se mantiene dentro del diálogo; el cierre explícito o con Escape lo devuelve al disparador. Las rutas directas de registro, inicio de sesión, verificación, reenvío, recuperación, restablecimiento e invitaciones se mantienen. La landing ofrece accesos diferenciados para iniciar sesión y registrarse, y comunica que cada pegatina NFC abre el hogar al que está vinculada. Conserva el token de acceso en memoria y la API gestiona la cookie de renovación. Permite seleccionar hogares y listas, crear hogares/listas y gestionar productos; consulta la lista visible cada 15 segundos y aplica mutaciones optimistas con reintento ante conflictos.
...
- La ruta autenticada `/` muestra un panel de hogares con recuentos de miembros, listas, pendientes y progreso, actividad reciente y accesos para crear hogares/listas o abrir NFC. Crear hogar y Crear lista abren formularios funcionales en sus secciones; si aún no existe ningún hogar, se conserva el flujo para crear el primero y su lista predeterminada. Los enlaces contextuales `/?household=...&list=...` siguen abriendo la lista solicitada.
...
- La PWA no encola ni sincroniza mutaciones offline. Android limita su cola offline a mutaciones de productos y no añade rutas HTTP nuevas. WebSockets y notificaciones push quedan fuera de este alcance.
