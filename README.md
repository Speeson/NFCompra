...
- Android ofrece pantallas de autenticacion estilo Grocery/Big Cart con welcome, login, signup, recuperacion por email, pantalla OTP de 6 cifras y restablecimiento con nueva contrasena/confirmacion en espanol, sustituyendo la imagen original por el logo de NFCompra. El OTP se valida contra la API antes de avanzar a la pantalla de nueva contrasena, y los errores JSON de la API se muestran como mensajes legibles para el usuario. Welcome incluye acceso por email y boton visual de biometria pendiente de implementacion. Los formularios de auth usan iconos Material en los campos y control de ojo para mostrar u ocultar contrasenas. El registro Android recoge nombre, apellidos, fecha de nacimiento, username, email, password y confirmacion de password, usa campos compactos con placeholder estable para que el texto no se corte al enfocar, sube el logo y amplia la tarjeta del formulario. Conserva los tokens de sesion y renovacion mediante Android Keystore. La pantalla autenticada consulta `/v1/me` para mostrar el nombre real del usuario en Inicio, incluye un banner superior compacto con degradado verde, titulo centrado, campana superior con panel desplegable de notificaciones y flecha de vuelta en pantallas secundarias, status bar verde, navbar inferior flotante con el mismo degradado para Inicio, Hogares, Listas, Catalogo y Perfil, contenido interno estilizado con los colores web verde oscuro, lima, fondo claro y textos oscuros, pantalla de Catalogo con categorias reales de `/v1/product-categories` en tarjetas visuales, resumen de hogares/listas, seleccion de hogares aunque no tengan listas, creacion manual de la primera lista, renombrado, vaciado total de lista, eliminacion de listas y gestion de productos, con estados de carga, error y datos, y reintento ante conflictos.
