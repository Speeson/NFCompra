...
- Hito 3B (base Android): Room conserva hogares, listas, productos y operaciones pendientes en una base aislada por cuenta; los tokens continúan en Android Keystore y no se guardan en la base. Cada colección persiste la presencia y fecha de su último snapshot correcto, por lo que un snapshot vacío sigue disponible sin conexión y un primer fallo sin snapshot muestra error en lugar de inventar una lista vacía. La lista visible observa continuamente la caché y conserva cambios locales pendientes durante los refrescos. La sesión de compras cancela sus peticiones y observadores y libera su base antes de sustituirse tras cerrar sesión o cambiar de cuenta. Crear, editar o borrar un producto actualiza Room y añade una operación `pending` con UUID y secuencia persistente sin esperar una llamada de sincronización. La ejecución ordenada de esa cola y la resolución de conflictos se incorporarán con WorkManager en la siguiente tarea.
