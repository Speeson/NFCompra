...
- Hito 3B (base Android): Room conserva hogares, listas, productos y operaciones pendientes en una base aislada por cuenta; los tokens continúan en Android Keystore y no se guardan en la base. Los inicios de sesión, peticiones y renovaciones quedan vinculados a una generación de sesión, por lo que una respuesta tardía de una cuenta anterior no puede reintentarse con credenciales nuevas ni reemplazarlas. Cada colección persiste la presencia y fecha de su último snapshot correcto; las cascadas eliminan también los metadatos hijos, de modo que un snapshot vacío válido sigue disponible sin conexión y un primer fallo sin snapshot muestra error. Hogares, listas y productos visibles proceden del estado Room fusionado, que conserva cambios locales pendientes durante los refrescos. Las sesiones simultáneas de una cuenta comparten la base mediante propiedad contada y solo la última liberación la cierra. Crear, editar o borrar un producto actualiza Room y añade una operación `pending` con UUID y secuencia persistente sin esperar una llamada de sincronización. La ejecución ordenada de esa cola y la resolución de conflictos se incorporarán con WorkManager en la siguiente tarea.
