...
- **Catalog cleanup**: migration `0011_catalog_category_merge_and_icons.sql` consolidates small duplicate seed categories into the Mercadona-style category set, moves non-duplicate products, deactivates exact duplicate products after copying favorites, and updates generic product/category icon keys. Migration `0012_catalog_icon_priority_fixes.sql` fixes icon priority for existing catalog rows: diapers/panales before bread, soft drinks before sugar/coffee, water/drink/coffee split, hair care before bread, and specific parafarmacia icons. Catalog SQL seeds now infer product-specific icon keys from product names before category fallback; verified seed imports have 0 generic product icons.
