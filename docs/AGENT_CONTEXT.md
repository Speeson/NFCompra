...
- **Android biometric unlock**: optional local-only setting in Profile > Ajustes. It uses AndroidX Biometric with `BIOMETRIC_STRONG`, stores only the enabled account id in private SharedPreferences, gates protected content on cold startup when a persisted session exists, keeps NFC/deep-link destinations pending until unlock, and explicit logout clears the local biometric association.
