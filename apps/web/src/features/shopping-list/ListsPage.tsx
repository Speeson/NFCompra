...
export function ListsPage({ onNavigate, startCreating = false }: { onNavigate(path: string): void; startCreating?: boolean }): JSX.Element {
  ...
  return <section className="route-page"><header className="route-page__header"><div><p className="eyebrow">Mis listas</p><h1>Listas activas</h1></div>{homes.length ? <button className="button" type="button" onClick={() => setCreating(true)}>Nueva lista</button> : null}</header>
