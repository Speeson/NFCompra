...
export function ListDetailRoute({ listId, currentUserId }: { listId: string; currentUserId: string }): JSX.Element {
  const households = useQuery({ queryKey: householdQueryKey, queryFn: fetchHouseholds });
  const homes = households.data ?? [];
  const listQueries = useQueries({ queries: homes.map((home) => ({ queryKey: listQueryKey(home.id), queryFn: () => fetchLists(home.id) })) });
  if (households.isPending || listQueries.some((query) => query.isPending)) return <main className="route-page"><p role="status">Cargando lista…</p></main>;
  const household = homes.find((_home, index) => (listQueries[index].data ?? []).some((list) => list.id === listId));
  if (!household) return <main className="route-page"><p role="alert">No se encontró esta lista.</p></main>;
  return <ShoppingListRoute currentUserId={currentUserId} requestedHouseholdId={household.id} requestedListId={listId} />;
}
